// DESCRIPTION:
// The 26 letters of the English alphabets are randomly divided into 5 groups of 5 letters with the remaining letter
// being ignored. Each of the group is assigned a score of more than 0. The ignored letter always has a score of 0.
//
...
function nameScore(name) {
    let count = 0;
    let res = {};
    let name1 = name.replace(/ /gi, '').toUpperCase().split('');
    for (let key in alpha) {
        for (let i = 0; i < name1.length; i++) {
            if (key.indexOf(name1[i]) !== -1) count += alpha[key]
        }
    }
    res[name] = count;
    return res;
